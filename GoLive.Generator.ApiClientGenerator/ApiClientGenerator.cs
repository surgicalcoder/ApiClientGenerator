...
public class ApiClientGenerator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        var controllerDeclarations = context.SyntaxProvider
            .CreateSyntaxProvider(
                static (s, _) => Scanner.CanBeController(s),
                static (ctx, _) => GetControllerDeclarations(ctx))
    ...
    {
        return text.Path.EndsWith("ApiClientGenerator.json");
    }
        ...
        IEnumerable<AdditionalText> configurationFiles, SourceProductionContext context)
    {
        ...
        namespaceBuilder.AppendLine("using System.Threading.Tasks;");
        namespaceBuilder.AppendLine("using System.Net.Http.Json;");
        namespaceBuilder.AppendLine("using System.Collections.Generic;");
        namespaceBuilder.AppendLine("using System.Threading;");
        namespaceBuilder.AppendLine("using System.Diagnostics.CodeAnalysis;");
        namespaceBuilder.AppendLine("using System.Text.Json.Serialization.Metadata;");
        namespaceBuilder.AppendLine("using Microsoft.Extensions.Primitives;");
        namespaceBuilder.AppendLine("using Microsoft.AspNetCore.Http;");

        if (config.OutputJSONSourceGenerator)
        {
            namespaceBuilder.AppendLine("using System.Text.Json.Serialization;");
        }

        if (config.ResponseWrapper.Enabled)
        {
            namespaceBuilder.AppendLine("using System.Net;");
            namespaceBuilder.AppendLine("using System.Net.Http.Headers;");
        }
        ...
        if (config.Includes != null && config.Includes.Any())
        {
            ...
            {
                namespaceBuilder.AppendLine($"using {s};");
            }
        }
        ...
        if (config.URLGeneration.Count > 0)
        {
    ...
    private static RouteGeneratorSettings LoadConfig(IEnumerable<AdditionalText> configFiles)
    {
        ...
        if (config.URLGeneration is { Count: > 0 })
        {
            foreach (var urlGenerationSetting in config.URLGeneration)
            {
                urlGenerationSetting.Filename = Path.GetFullPath(Path.Combine(configFileDirectory, urlGenerationSetting.Filename));
            }
        }
    ...
    private static void SetUpSingleApi(RouteGeneratorSettings config, ControllerRoute controllerRoute, SourceStringBuilder source, SourceStringBuilder urlSourceBuilder)
    {
        ...
        using (source.CreateBracket())
        {
            ...
            {
                var byteReturnType = action.ReturnTypeName == "byte[]";
                ...
                var containsFileUpload = action.Mapping.Any(f => f.Parameter.FullTypeName == IFormFile_Q);
                ...
                var useCustomFormatter = config.CustomDiscriminator;
                ...
                var returnType = config.ResponseWrapper.Enabled switch
                {
                ...
                using (source.CreateBracket())
                {
                    ...
                    {
                        var idempotencyValue = string.Empty;

                        var actionHasIdempotency = action.AllAttributes.Any(attr => config.Properties.IdempotencyRequired.TryGetValue(attr, out idempotencyValue));

                        var controllerHasIdempotency = !actionHasIdempotency && controllerRoute.AllAttributes.Any(attr => config.Properties.IdempotencyRequired.TryGetValue(attr, out idempotencyValue));
    ...
    private static void CreateURLOutput(SourceStringBuilder urlSourceBuilder, ControllerRoute controllerRoute, ActionRoute action, CaseInSensitiveDictionary actionValues, string routeString)
    {
        ...
        {
            methodName = controllerRoute.Name;
        }
