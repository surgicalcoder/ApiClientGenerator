...
public readonly struct QueryString : IEquatable<QueryString>
{
    ...
    /// <returns>The resulting QueryString</returns>
    public static QueryString Create(string name, string value, bool UrlEncode)
    {
    ...
    /// <returns>The resulting QueryString</returns>
    public static QueryString Create(IEnumerable<KeyValuePair<string, string?>> parameters, bool UrlEncode)
    {
        var builder = new StringBuilder();
        var first = true;
        foreach (var pair in parameters)
        {
            AppendKeyValuePair(builder, pair.Key, pair.Value, first, UrlEncode);
            first = false;
        }
    ...
    /// <param name="parameters"></param>
    /// <returns>The resulting QueryString</returns>
    public static QueryString Create(IEnumerable<KeyValuePair<string, StringValues>> parameters, bool UrlEncode)
    {
        var builder = new StringBuilder();
        var first = true;
        ...
        foreach (var pair in parameters)
        {
            ...
            {
                AppendKeyValuePair(builder, pair.Key, null, first, UrlEncode);
                first = false;
                continue;
            }
            ...
            {
                AppendKeyValuePair(builder, pair.Key, value, first, UrlEncode);
                first = false;
            }
        }
    ...
    /// <returns>The concatenated <see cref="QueryString"/>.</returns>
    public QueryString Add(string name, string value, bool UrlEncode = true)
    {
        ExArgumentNullExceptionExt.ThrowIfNull(name);

        if (!HasValue || Value.Equals("?", StringComparison.Ordinal))
        {
            return Create(name, value, UrlEncode);
        }

        var builder = new StringBuilder(Value);
        AppendKeyValuePair(builder, name, value, first: false, UrlEncode);
        return new QueryString(builder.ToString());
    }
    ...
    private static void AppendKeyValuePair(StringBuilder builder, string key, string? value, bool first, bool UrlEncode)
    {
